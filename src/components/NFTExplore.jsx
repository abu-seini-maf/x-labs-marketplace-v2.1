...
import { useNFTsByContracts }from 'hooks/useNFTsByContract';
...
function NFTExplore({category, setCategory}) {
  const fallbackImg =
...
//   const { NFTSearch, totalNFTs, fetchSuccess } = useNFTSearch(inputValue);
  ...
  const { NFTs, NFTsTotal, fetchSuccess } = useNFTsByContracts(NFTCollections);
  ...
  return (
    <>
      <div>
        ...
        <div>
        Total NFTS: {NFTsTotal}
        </div>
        <div style={styles.topfiltration}>
            {NFTCollectionsCategory?.map((item, index) => (
